...
- **F#10** — [Lighten the library PDF viewer's heavy native toolbar](#f10--lighten-the-library-pdf-viewers-heavy-native-toolbar) · `medium` · **NOT STARTED** · decided: Option B (pdf.js vendor — needs network + browser)
- **F#11** — [Paper-header design overhaul (cohesive pod, unified bib chrome, page picker)](#f11--paper-header-design-overhaul-cohesive-pod-unified-bib-chrome-page-picker) · `medium` · **◑ PARTIAL (Phase 4b)** — `<BibEntryChrome>` + cohesive responsive pod + drag-to-library + text page-picker + header open LANDED; DEFERRED: editor Bibliography-panel single-source adoption (fast-follow) + PDF page-picker (needs F#10)
