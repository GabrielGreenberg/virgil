...
## Progress (updated 2026-05-26, session 7 — action menu diagnosis fixes complete)
...
1. **Sub-object anchored cards. [LIKELY-RESOLVED — session 7, 2026-05-26 — see ACTION-MENU-DIAGNOSIS.md cluster C7.]** D8 + H1 together unlocked the data path for "anchor a note/quote/todo to a listItem or exampleItem." Phase C of the action-menu diagnosis ground-truthed `listItem × Note`: sidecar correctly records `targetKind: "listItem"` and `textObjectIds: ["fa04"]`, and the panel card renders at the listItem's vertical position. The remaining open question is whether the *gutter marker* (separate from the panel card) sits next to the sub-object's row — Phase C's §7 q11 noted the gutter renders only one marker at a time, which is a separate (and likely pre-existing) rendering issue, not a sub-object anchoring failure. If gutter sparseness turns out to mask a sub-object positioning bug, the fix lives in `Marginalia.tsx`'s position computation (read `targetKind` to position by sub-object's `data-uuid` element).
