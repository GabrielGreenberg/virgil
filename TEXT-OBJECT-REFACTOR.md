...
## Progress (updated 2026-05-22, session 2)

Branch: **`text-object-refactor`** (6 commits landed, 5 remaining).
...
After commits 1–6: typecheck clean, full suite 151/159 passing (the 8 failures are pre-existing in `usePersistentState.test.ts`, unrelated).

### Remaining (5 commits)

Strict dependencies: D8 before E.

| # | Phase | Spirit (NOT a surgical patch — keep the architectural ambition) |
|---|---|---|
| 7 | D8 | Full restructure of `Link.anchor`: rename `type: "anchor"` → `"textObject"`; add `targetKind: TextObjectKind`; rename `paragraphIds` → `textObjectIds`. `isModeB(link)` becomes `link.anchor.type === "textObject" && link.anchor.targetKind === "linkedRange"`. `migrateCardLinks` extended with one-shot read-side migration: infer `targetKind` by resolving the node id in the doc (sub-objects get their actual kind; missing → `"paragraph"` + console.warn). Mode B/A distinction collapses to a derived check. |
| 8 | D5 + D6 | Float collapse: delete `ParagraphFloat.tsx`, `HeadingFloat.tsx`, `ListFloat.tsx`, `SelectionFloat.tsx`, `TexBlockFloat.tsx`, and `selection-floats.ts`. Their bodies relocate as registry-registered components via `registerFloatBody`; chrome is the unified `TextObjectFloat`. **Chrome unified; body sync stays per-kind** — abstracting CodeMirror-vs-TipTap sync would create false unification (TexBlockFloat keeps its CodeMirror sync internally). `cardContext` becomes a body-component concern. Same commit: drop-spec collapse — delete `paragraph.ts`/`heading.ts`/`selection.ts` from `drop-mode/specs/`; new `textobject.ts` consumes `TextObjectTransportPayload` and dispatches through `dropAdapterFor`. After D5+D6 the `case "textobject"` dispatcher reads `meta.floatBodyComponent` directly (the legacy routing in D10 goes away), and the transitional `case "list"` can finally be deleted. |
| 9 | E | Selection hydration + multi-paragraph linkedAnchor LaTeX round-trip. `hydrateSelectionToTextObject` (already exists in C2) wired at three commit sites: popout commit, card-anchor commit, drop-mode commit. The session-only "selection float" category is gone. New paired markers `\vlid{anchorId}…\vlidend{anchorId}` introduced in the parser + serializer (this is a NEW LaTeX-level round-trip — the mark was app-state-only before, persisted via sidecar `anchorText`). Parser is defensive: unmatched `\vlid` logs a console.warn and recovers via sidecar `textSnapshot`. Sidecar `paragraphId` (single) → `paragraphIds` (array). The transitional `case "selection"` in `floating-cards.tsx` and the TextObjectGrabHandle's `selection:<id>` fallback both go away. |
| 10 | F | Final migration audits. `\vxid` lazy assignment confirmed working for legacy docs; `migrateCardLinks` covers every consumer; orphan link surfacing; no silent data loss. **Doc-aware sweep of remaining legacy popout keys** (D10 deferred this — `list:<uuid>` resolved by walking the doc for the actual list kind → `textobject:bulletList:<uuid>` / `textobject:orderedList:<uuid>`; `example:<uuid>` disambiguated against the Examples panel-card keys; missing nodes dropped with console.warn). After F, no transitional legacy cases remain in the dispatcher. Optional: audit the function-parameter `paragraphId` uses in card-creation APIs (`addNote(paragraphId, …)` etc.) and the `EditorHandle.ensureParagraphUuid*` methods — they are kind-agnostic now and could rename for consistency. No doc-format version sentinel needed (migration is additive). |
