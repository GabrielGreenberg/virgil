...
## Progress (updated 2026-05-26, session 9 — cohesive grab-handle mop-up landed, visibility regression open)
...
0. **🔴 Grab-handle visibility regression. [OPEN — session 9, 2026-05-26.]** After session 9 landed (`f6a9b93`), grab bars disappeared completely in the live preview. The hotfix attempt within the same commit (inline portalRoot resolution + z:20) did NOT restore them. Architecture is sound on paper (verified by typecheck + new unit tests pass; code review traced the placement/portal math through). Likely culprits surfaced in the investigation: (a) stacking context trap in `paper-render` or an ancestor that isolates the handle's z below editor content; (b) the portal wrapper's `inset: 0` not producing usable dimensions in the actual layout; (c) a timing/render path where placements stay stale; (d) elementFromPoint not finding `[data-uuid]` for some reason. Full investigation plan with priority-ordered hypotheses, console diagnostics to run, fallback options (including a temporary revert path that preserves the architecture): [docs/memos/GRAB-HANDLE-VISIBILITY-FOLLOWUP.md](docs/memos/GRAB-HANDLE-VISIBILITY-FOLLOWUP.md). **Read first before touching the grab handle.**
