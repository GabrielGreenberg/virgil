...
After commits 12–13: typecheck clean, full suite 222 passing / 8 pre-existing baseline failures.

### Current state cheat-sheet (read before touching code)

**Where things live:**
- Registry SSOT: [src/text-objects/text-object-registry.ts](src/text-objects/text-object-registry.ts) — 16 kinds; each has `isSubObject`, `isAtomBlock`, `decorationSafety`, `parentKind?`, `dropAdapter`, `floatBodyComponent`, `initialFloatSize?`, `collectMoveSource?`.
...
**Handle placement math.** `computeHandleLeftEdge` in [src/text-objects/handle-layout.ts](src/text-objects/handle-layout.ts): `max(elDOM.left - decorationSafety - HANDLE_GAP, editorColumnLeft - HANDLE_GAP)`. The caller in `TextObjectGrabHandle` derives `editorColumnLeft` from `editor.view.dom.getBoundingClientRect().left` (the `.ProseMirror` element), NOT from the anchor DOM — this is what lets sub-object handles indent past their own DOM edge into the decoration zone.
