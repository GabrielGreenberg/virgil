...
## Progress (updated 2026-05-25, session 6 — action menu + hover zone, ongoing)
...
**Where things live:**
- Registry SSOT: [src/text-objects/text-object-registry.ts](src/text-objects/text-object-registry.ts) — 16 kinds; each has `isSubObject`, `isAtomBlock`, `decorationSafety`, `chromeAnchor` (post-H4: `"text-top" | "block-top"`), `parentKind?`, `dropAdapter`, `floatBodyComponent`, `initialFloatSize?`, `collectMoveSource?`, `actions` (post-I1 includes `duplicate` + `delete` + the prior `archive`).
- **Card lifecycle SSOT (post-I1):** [src/panels/card-lifecycle-registry.tsx](src/panels/card-lifecycle-registry.tsx) — per-CardKind `{ clone(sourceId): newId \| null; delete(id): void }`. Per-doc (sidecar hooks are per-doc), populated by EditorPane via `useCardLifecycleApi(registry)`. Consumed by the duplicate/delete walkers via `getCardLifecycle(kind)`. Sibling of `PANEL_REGISTRY` / `LINK_REGISTRY`.
- **Duplicate / delete walkers (post-I1):** [src/text-objects/duplicate-slice.ts](src/text-objects/duplicate-slice.ts) (remints uuids + inline-atom ids + linkedAnchor marks; clones sidecar entries via the lifecycle registry) and [src/text-objects/delete-range.ts](src/text-objects/delete-range.ts) (enumerates sidecar-bearing elements in a range and calls `lifecycle.delete`). Both contain zero per-kind branches; the only kind-aware code is a 2-entry `INLINE_ATOM_CARDS` map literal.
...
- Grab handle: [src/text-objects/TextObjectGrabHandle.tsx](src/text-objects/TextObjectGrabHandle.tsx) — ONE editor-mounted component. Hover-driven (post-H2). Renders array of placements via inner `<GrabHandleRender>`. Listener attaches to `document` (post-I2), gated by `cache.containsHoverZone`. Resolver is zone-first + coord-clamp + ancestor-walk + vertical-contain guard (post-I2).
...
9. **Archive in the lifecycle registry.** The `archive` dispatch path in [drag-handle-actions.ts](src/components/editor-layout/card-actions/drag-handle-actions.ts) still uses ad-hoc deps (`archiveContent` / `updateArchiveSnippet` / `addArchiveTextObjectId` / `setSelectedArchiveId` / `pinRecentlyAddedArchive`) rather than going through `getCardLifecycle("archive")`. The shape mismatch: archive's "create" takes content (not a sourceId — there's nothing to clone), and the existing path cascades into panel-focus side effects. Fold either by extending `CardLifecycle` with `create(content): id` (and a separate ergonomic context for panel-focus) OR by treating archive's create as its own ergonomic API and leaving lifecycle as the clone/delete-only registry. Either way, the goal is the same: archive becomes a peer of duplicate/delete in the same SSOT, not a parallel rail. Explicit TODO comment marks the spot.
10. **Cloned card link rewireup.** I1's `cloneCard(id)` produces a copy with `links: []` so the new sidecar entry doesn't drag stale anchor / paragraph references. The duplicate walker rewrites the inserted slice's `linkedAnchor` marks with new `anchorId` + new `linkCard` pointing at the cloned card, so editor → card jump-to works. Card → editor jump-to (which reads the card's own `links[]`) won't find the new mark until something repopulates the cloned card's `setTextAnchorLink` / `addTextObjectLink` from the new mark + its host paragraph. Natural follow-up: after the dispatcher inserts the cloned slice, walk it and call each card's `setTextAnchorLink` / `addTextObjectLink` via a `bindAnchor` op added to `CardLifecycle`. The walker has the info it needs; the rewireup just hasn't been wired.
11. **Mode A paragraph-link orphans on delete.** I1's `delete-range.ts` walker cleans up sidecar entries for inline atoms (footnote / citation) and linkedAnchor-marked cards (note / highlight / comment / cutter-comment / cutter-suggestion) inside the deleted range. It does NOT clean up Mode A paragraph-anchor links on todos / quotations / examples / archive cards that pointed at the deleted paragraph — those become "remembered" stale links. The existing `virgil-anchor-orphaned` event sweeps Mode B; Mode A doesn't have an equivalent. Add a parallel `virgil-paragraph-orphaned` sweep (or extend the existing event) so deleting a paragraph nukes the dead Mode A links uniformly.
