...
## Progress (updated 2026-05-22, session 3)

Branch: **`text-object-refactor`** (8 commits landed, 3 remaining).
...
After commits 1–8: typecheck clean, full suite 157 passing / 8 pre-existing failures (`usePersistentState.test.ts`, unrelated baseline).

### Remaining (3 commits)

Strict dependencies: D8 before E. (D8 has landed.)

| # | Phase | Spirit (NOT a surgical patch — keep the architectural ambition) |
|---|---|---|
| 9 | E | Selection hydration + multi-paragraph linkedAnchor LaTeX round-trip. `hydrateSelectionToTextObject` (already exists in C2) wired at three commit sites: popout commit, card-anchor commit, drop-mode commit. The session-only "selection float" category is gone (`SelectionFloat.tsx`, `selection-floats.ts`, `drop-mode/specs/selection.ts`, `case "selection"` in floating-cards.tsx, TextObjectGrabHandle's `selection:<id>` fallback all deleted). A new `linkedRange` float body component built and registered. New paired markers `\vlid{anchorId}…\vlidend{anchorId}` introduced in the parser + serializer (this is a NEW LaTeX-level round-trip — the mark was app-state-only before, persisted via sidecar `anchorText`). Parser is defensive: unmatched `\vlid` logs a console.warn and recovers via sidecar `textSnapshot`. Sidecar `paragraphId` (single) → `paragraphIds` (array). When E lands, the only transitional cases left in the dispatcher are `case "list"` (F's doc-aware sweep) and `case "example"` (intentionally stable panel-card prefix). |
| 10 | F | Final migration audits + legacy-key sweep. **Doc-aware sweep of remaining legacy popout keys** that D10 deferred: `list:<uuid>` resolved by walking the doc for the actual list kind → `textobject:bulletList:<uuid>` / `textobject:orderedList:<uuid>`; `example:<uuid>` (in-editor popout collision) disambiguated against the Examples panel-card prefix; missing nodes dropped with console.warn. After F's sweep, `case "list"` can finally be deleted from the dispatcher. **`targetKind` inference upgrade** (deferred from D8): for any legacy Mode A link whose textObjectId resolves to a sub-object kind, upgrade `targetKind` from the default `"paragraph"` to the actual kind. Pre-D9 data has no sub-object anchors so this is forward-looking defense; the upgrade walks the doc on first load. `migrateCardLinks` coverage audit — confirm every link-bearing card hook routes through it; orphan link surfacing with console.warn. `\vxid` lazy-assign confirmed working for legacy docs. Optional: audit function-parameter `paragraphId` uses in card-creation APIs (`addNote(paragraphId, …)`) and `EditorHandle.ensureParagraphUuid*` methods — kind-agnostic now, could rename for consistency. No doc-format version sentinel needed (migration is additive). Smaller items to fold in: move `PER_KIND_FLOAT_SIZE` from `TextObjectGrabHandle.tsx` into a `meta.initialFloatSize?` registry field; extend `SourceMissingBanner`'s `FloatSourceKind` union to cover more kinds or make it derived from `TextObjectKind`. |
| 11 | G | Sample extension + agent-docs refresh + dev-preview walkthrough. Extend `samples/annotation-history/` with graphic-in-list-item, graphic-in-example-item, multi-paragraph linkedAnchor, sub-object card anchors. Walk the §13 checklist end-to-end (pop out every kind, drop into same-parent / incompatible / top-level contexts, anchor cards to sub-objects + atom blocks, reload). Refresh `docs/agents/main-text.md` (Block nodes + Link architecture sections, `textObject` schema group, Mode A/B collapse), `docs/agents/architecture.md` (SSOTs table + MIME map + popout-key prefixes + float collapse note), `docs/agents/glossary.md` (TextObject entry, Mode A/B collapse), `docs/agents/overview.md` (text-objects in core concepts). **Phase D9 functionality verification** belongs here: the data model already supports cards on any kind (D8's `targetKind` field), but G's walkthrough is the first time sub-object anchoring gets exercised end-to-end. Verify `MarginaliaAnchorGuard`'s populator iterates by `isInGroup("textObject")` rather than node-name; verify marginalia layout positions correctly when anchoring to a sub-object. Add unit tests for `drop-mode/specs/textobject.ts` covering the source-kind × target-context matrix (deferred from D6). |
