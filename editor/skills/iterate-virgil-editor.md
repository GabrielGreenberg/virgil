...
- **Cross-skill edge** — e.g. footnote requesting a missing citation (should file a missing-bibkey **todo card** through Workflow B, not fabricate a `\cite*{}` and not hand-append an `ai-requests.json` row — see [_ask-shape.md](_ask-shape.md) §4); answer-note-request that the markdown classifies as wanting a doc edit (should produce a suggestion, not a note).
