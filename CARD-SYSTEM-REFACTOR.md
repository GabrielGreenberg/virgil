...
### Session 14 — BATCH-2: A9 + A2-rest MERGED (BATCH 2 → 2/3) (2026-06-10)
- **A2-rest ✅ merged to `main`** (`e39ffff`, `--no-ff`; main 52 ahead of origin, **not pushed**). Commits A–D + review-nits; **tsc clean, 612 tests (59 files)**. The anchor/link-layer consolidation (cards only; A2-B1 had landed `EntityKind=CardKind`). Delivers 4 of the 5 A2-rest items: **`cardKindFromRecord(record, panel)`** — the comment/suggestion read-side classifier (panel-disambiguated; the INVERSE of A9's morph write-side, intentionally NOT merged) replacing the scattered `.kind === "suggestion"` switches in `findEntity`/`useTextHoverBridge`; **`isInlineAtomCardKind`** (explicit footnote/citation literal — not cleanly facet-derivable since highlight shares `markerType:null`); the single-sourced **legacy-token crosswalk** (`src/cards/legacy-token-crosswalk.ts`; R-C: tokens byte-identical, zero disk/CSS migration; frozen-table pin test); **derived `ANCHOR_CLICK_ROUTES` + `entityKindToAnchorKind`** (from `panelForCardKind`/`markerType`, with the **R-B cutter anchor-tint split** preserved + a contract pin-test). **Independent adversarial review (Workflow `wf_d5844813-b7d`): GO-WITH-NITS** — the synthesizer proved each commit equivalent to its pre-A2 literals *case-by-case* (no hover/click mis-route, no token drift, keystroke sanctity intact, O(1) registry reads); 4 LOW/INFO nits, 2 folded (the crosswalk frozen-table pin + an unreachable-fallback dev-guard).
  - **The 5th item — retire `EntityCollections` / fold onto `CardFloatCtx` — was DEFERRED to A3** (its consumer). A standalone A2 fold isn't clean: `PoppedCardDeps` uses slot names `todoItems`/`reportCards` (vs `EntityCollections`' `todos`/`reports`) and `examples: ExampleInfo[]` keyed on `exampleId` (vs the `{id}[]` the EditorPane literal maps via `_examplesAsEntities`). A3 does **fold-then-consume**, resolving the examples-mapping + slot reconciliation in its own context. A-D is a coherent mergeable partial (review-confirmed): `EntityCollections` still exists + is consumed; the now-unused `_c` collections param in `cardKeyForEntity`/`entityKindToAnchorKind` is an idiomatic discard, not dead code.
- **A9-impl ✅ merged to `main`** (`f68fd79`, `--no-ff`; **not pushed**). 6 commits (A–E + review-nits); **tsc clean, 605 tests (57 files)**. The first BATCH-2 arena. Delivers:
  - **Polymorphic morph chevron** — generalized from the revisions-only `convertCard` to ALL 4 panel pairs (note↔highlight, revision-comment↔revision-suggestion, cutter-comment↔cutter-suggestion, report↔report-request), **docked + popped + omni**, via a kind-agnostic `EditorPane.convertCardWithRemap(fromKind,id)` that reads `morph.to`/`lossy` from `CARD_REGISTRY`, optionally awaits a lossy confirm, dispatches the owning hook's `convertCard` (→ a pure transform registered via `registerCardMorph`, runtime-leaf), then `remapCardPopKey` in lockstep so **a popped card survives the morph** (verified LIVE: popped revision-comment → revision-suggestion, float survived z1201, key remapped, body re-rendered with suggestion fields; + a cutter contract test pins both directions + rect-follow + no-op-when-not-floated). New `CardMeta.morph` field + `canMorph` predicate + `assertMorphCoverage` (reciprocity + panel-share).
  - **Two-class typography (N2)** — new `CardMeta.bodyClass` ("borrowed" serif/15 = footnote/archive/example; "sans" Inter/12 = everyone incl **report** per R11); `DEFAULT_PANEL_TYPOGRAPHY` derived from it + `assertPanelTypographyCoverage`. The review-nit fix threads the panel size onto `BorrowedMainText` via **`--editor-font-size`** (the var the global `.tiptap p` rule resolves — the root-cause the review caught) so borrowed bodies honor 15px + the size-stepper.
  - **Borrowed-from-main-text display (C1)** — new `src/components/BorrowedMainText.tsx` (read-only TipTap, `editable:false`, zero `editor.on`) renders borrowed bodies with real atoms (citation/`\ref`/math); content-keyed value-sync (`setContent(…,{emitUpdate:false})`) — keystroke sanctity verified flat (live `emitCount` Δ0). Used the ratified schema FALLBACK (own read-only extension list; the shared-`borrowed-schema.ts` dedup deferred → MEMO_BUG_BACKLOG #11).
  - R5 (dead `ac.props` aria removed), R14 (one-way `+note` button replaced by the bidirectional chevron).
- **Independent adversarial review (Workflow `wf_cae739e9-119`): GO-WITH-NITS** (6 lenses + synthesizer re-running tsc/605-tests). Both load-bearing risks came back CLEAN (morph-remap float-survival; `BorrowedMainText` keystroke sanctity). It caught **2 HIGH that passed tests but missed the ratified intent** — the borrowed-body typography wasn't actually rendering at 15px (missing `--editor-font-size` write; stepper inert) and the example body froze at first mount (no value-sync) — plus a MEDIUM data-loss edge (`useNotes.convertCard` missing `pristine.markDirty`). **All 3 folded** into commit `2fd45fa` before merge; verified at source (fix mirrors the proven `RichTextField` pattern).
- **A9 follow-ups (deferred):** the borrowed-body **Examples-panel visual typography walk** is the one item I couldn't confirm headlessly (the backgrounded preview won't surface the Examples panel column) — recommend a manual 15px/live-edit check; note→highlight is offered for paragraph-only Mode-A notes (→ an empty tintless highlight) — a chevron-gating UX call (→ A2/A6); the hardcoded revision `kindOptions` vs derived (SSOT nit); `chrome-config` zero-consumer defs (→ A8); `bib`/`ai` full `FloatChrome` (→ a later thread).
- **Next: A3** (the last BATCH-2 arena) — creation pipeline (3 entry points → one), pristine-card consolidation, lifecycle ratification (4 permanent gaps documented; only `archive` delete-cascade live, R18), owns `popCardAtAnchor` (the 33-ref `CardKind`/`cardPopKey` re-type), **AND now the deferred `EntityCollections`→`CardFloatCtx`/`CardCollections` fold-then-consume** (resolving the `examples`/slot-name mismatch in-context). Then BATCH 3 (A5 omni-reflow ∥ A6 ∥ A10-rest).
- **Session 14 paused here (context budget).** The next-session handoff — an explicit MANAGEMENT-session prompt (Workflows + dispatched chips, ultracode, the deep-architecture central rule, the proven cadence + hard guards) — is in **`MEMO_NEXT_SESSION_PROMPT.md`** (repo root). Resume by reading this entry + the seam-sweep §5/§6, then dispatching a fresh A3 `Plan` agent.

### Session 13 — A4 keystone (BATCH-1) + AF-follow + A1 MERGED → **BATCH 0 COMPLETE** (2026-06-09)
- **🎉 BATCH 0 is COMPLETE** — all 5 chips landed: A2-B1 (`6697ad6`) · A10-D1 (`77c7c5c`) · A8 (`33d9b4a`) · **AF-follow (`355c1ad`)** · **A1 (`4fff0be`)**. main HEAD `e7ead17`, **not pushed**.
- **A1 gardening ✅ merged to `main`** (`4fff0be`, `--no-ff`). 9 commits, **+49/−1879** net (a big GC); final **tsc clean, 594 tests**. Removed: the **R30 detached actions/formatting/menu toolbars** — KEY: at HEAD these were **already trigger-severed DEAD code, not a live feature** (AttachedPopover never rendered, the docked MenuBar stripped `onGrabStart`, no keybinding/menu/setting reached them), so the KILL was **pure GC with ZERO live capability lost** (the docked action vocabulary runs through the kept `ActionsStripButton`→`ActionsMenuPanel`; rotate via `onSetOrientation`→`ViewMenu`; both verified live in the dev-preview + the review). Also removed: `useComments`/`UserComment`/`CommentsState`, the `menuLocation` view pref, the inert `FloatWindow` `autoFitBody` grow-burst (+ the `Floatable.autoFitBody` field), ErrorCard dead lift-wiring (doubly dead post-A4 `isPoppable` lift-gate; `compressed = !expanded`), the grip-redesign dead-drag blocks (+ a R6 reintro note in `MEMO_BUG_BACKLOG.md`), `floating-toolbar-shell.tsx` trimmed to `ToolbarOrientation` (R4), `snap-grid.ts` (242-line orphan), the dead `kind:"toolbar"` focus-ref union branch; 3 stale doc-comments reworded. **Ratified dispositions:** R2 (no `menuLocation` strip — structural-spread load), R3 (`chrome-config.ts` `isActionVisible`/`CALLBACK_TO_ACTION_KIND` defs LEFT for the A8 sweep), R5 (`useAnchoredCard` dead `ac.props` aria DEFERRED to A9). **Independent adversarial review (Workflow `wf_3de2cc6f-0a5`): GO-WITH-NITS** (6 lenses incl. a headline R30-capability lens that PROVED no live action/format/rotate lost + a synthesizer re-running tsc/tests/grep-gates; zero CRITICAL/HIGH). The load-bearing nits (snap-grid orphan, the toolbar union branch, the A1-introduced stale section header, the R6 path typo) were folded into commit 9 before merge. **Process slip (fixed):** an A1 `git add -A` accidentally committed the review's temp `.tmp-a1-review.mjs` into the merge — removed in a follow-up `chore` commit (`e7ead17`). Lesson: stage explicit paths or `rm` the temp BEFORE any `git add -A`.
- **AF-follow ✅ merged to `main`** (`355c1ad`, `--no-ff`; **not pushed**). Lands real per-kind `Floatable.snapshotForStack` on every card float and **deletes the legacy stack path** (`src/lib/stack/resolve-card.ts` — `resolveCardData`/`cardKeyPrefixToStackKind`), unblocking A1's stack-path deletion (GAP-8 closed). Commits a–e; final state **tsc clean, 594 tests (56 files), grep-gate zero**. The `cardFloatable` helper's `snapshotForStack` opt is now **mandatory** (a compile-time guard against a missed kind); each stackable builder closes over the record it already resolved for `renderBody` and calls `snapshotCard(<stackKind>, rec, source)` (the kept serialization primitive); the single EditorPane `virgil-stack-drop` consumer repointed to `CARD_REGISTRY[kind].toFloatable(id, popoutsDeps).snapshotForStack(source)` (mirrors `FloatHost.resolveFloatable`; text-object branch byte-for-byte intact). Ratified: footnote=R1-B (shim from `FootnoteInfo`, `content` round-trips), example=R2-null (preserved; no reachable `ExampleRef`), citation=R3 (bib sidecar from `ctx.bibEntries`). New `src/cards/__tests__/float-snapshot.test.ts` (17 cases incl. deep-clone isolation). **Independent adversarial review (Workflow `wf_e86e98ef-767`): GO-WITH-NITS** (6 lenses + synthesizer re-running tsc/tests/grep-gate; per-kind parity verified line-by-line against the *deleted* resolver; zero CRITICAL/HIGH; 3 LOW nits folded into commit e). **bib/ai → full `FloatChrome` DEFERRED** (orthogonal; `bareWindow` works; fold into A9). **Two AF-follow behavior notes:** the footnote builder now sources content from the live editor (`getFootnotes() ?? ctx.footnotes`) — strictly more current than the deleted sidecar read, sanctity-safe (one-shot read on the drop gesture only); `example` stays `stackable:true` in the registry while its float snapshots `null` (inert — `stackableCardKinds()` has zero consumers; reconcile when example stacking lands).
- **A4-impl ✅ merged to `main`** (`90c050e`, `--no-ff`; **not pushed**). The selection ⟂ expansion split (N1) is **complete and landed** — Commits A–H, each tsc + tests green; final post-merge state **tsc clean, 577 tests (55 files)**.
  - **Commit F** — the two axis-pure header controls in `panel-primitives.tsx`'s unified header: a one-click **expand chevron** (`CardExpandChevron`, docked-only, left of `CardKindHeader`/the A9 morph slot, `stopPropagation` on click+mousedown so it never bubbles to the body `onActivate` or the lift; canonical WAI-ARIA disclosure — `aria-expanded`+`aria-controls`(→ a `display:contents` body-id wrapper)+label; rotation tracks `!isCollapsed`) → `useAnchoredCard.onToggleExpanded` (axis-pure). A one-click **docked popout button** gated on a NEW registry SSOT **`CardMeta.poppable`** (only `error`=false) via the `isPoppable` predicate; uses the same AF Seam-1 `toggleAtAnchor` path as the lift; works collapsed. `registerCardFloatable` now reads `.poppable` (was a hardcoded `=== "error"`). `onToggleExpanded` threaded `useAnchoredCard` → `EditableCard` → all 13 cards (PanelCard stays store-agnostic).
  - **Commit G** — ErrorCard panel-local expand axis (R5): `error` is non-anchored (no cardStore slot), so `ErrorsPanel` owns an `expandedIds` set (`expandError` idempotent body-click set-true + `toggleError` chevron); `compressed = !expanded && !isPoppedOut` (was `!selected`). The omni error surface (`buildErrorOmniItems`) is a 2nd consumer — `omni-host` owns its own surface-local set.
  - **Commit H (review nits)** — **`isPoppable` is now the single registry SSOT gating BOTH the docked popout button AND the drag-lift** (`onWrapperMouseDown`): closes a pre-existing leak (also on main) where dragging an `error` header lifted it into a blank ghost float. New **`anchored-card-store.test.ts`** (7 cases) pins the axis-purity invariant so a future re-weld fails loudly. Two stale comments corrected to the N1 model (EditorLayout click-away, NoteCard).
- **Independent adversarial review (Workflow `wf_fe0364fe-464`): GO-WITH-NITS.** 8 diverse skeptic lenses → adversarial refutation of every CRITICAL/HIGH → a synthesizer that **independently re-ran `tsc` (0) + the full vitest suite (570/570 at review time)** on the branch checkout (no branch switch; hard no-working-tree-mutation guard held). **Zero CRITICAL/HIGH survived.** The two load-bearing MEDIUM nits were folded into Commit H; the rest deferred (see below).
- **Dev-preview verified** on `doc_devtest`: keystroke sanctity **`emitCount` Δ0** typing 12 chars (version +12); **B1** expand chevron unfurls (aria false→true, body 68→255 chars) with the halo **not moving** + card stays unselected; **B2** one-click popout pops a **collapsed, unselected** card → float (z1200, 360w), docked source stays unselected; 68 chevrons + 68 docked popouts render; `aria-controls` resolves; no console errors.
- **Deferred A4 follow-ups (route to downstream chips):** (1) **R5 error-expansion global scoping** — lift the error `expanded` set to `EditorLayout` (alongside `selectedErrorId`) so the docked panel + omni share scope (currently per-surface; halo is already global) → A5/A10. (2) `useAnchoredCard` still emits `aria-expanded` in `ac.props` but **no card spreads `ac.props`** (dead; the chevron carries the canonical aria) → delete or adopt the spread (A1/A9). (3) `OrphanedFootnoteCard` keeps the pre-A4 welded `compressed = !isSelected` (no `ac`, no chevron) → panel-local axis like G, follow-up. (4) `expandedIds` (panel + omni) never pruned on dismiss/re-lint → small prune effect. (5) keyboard-nav-no-longer-auto-expands + `handleCardFocus` no-op → **conscious UX sign-off** + A5 expand-on-focus. (6) CitationCard draft chevron is a dead click while `isDraft` → optionally suppress.
- **Next: BATCH 2** (A3 ∥ A2-rest ∥ A9 — the morph chevron + two-class typography N2 + borrowed-display; A9 serialized after A4 on `panel-primitives.tsx`; A3 owns `popCardAtAnchor` + lifecycle ratification) → then BATCH 3 (A5 omni-reflow ∥ A6 ∥ A10-rest). See seam-sweep §5 for ordering + the contention map. **A1 follow-ups routed downstream:** `chrome-config.ts` `isActionVisible`/`CALLBACK_TO_ACTION_KIND` (now zero-consumer) → A8 sweep; the dead `ac.props` aria → A9; the pre-existing `onAdd*`-family dead pass-through props on `MenuBarContent` → A9/A3.

### Session 12 — Wave-3 BATCH-0 (3 chips) merged + A4 keystone A–E on a branch (2026-06-09)
- **BATCH 0: 3 of 5 chips ✅ merged to `main`** (each through the full audit→impl→**independent adversarial review**→`--no-ff` merge loop; main at `cdf6be1`, 6 ahead of origin, **not pushed**):
  - **A2-B1** (`6697ad6`) — `ANCHORED_CARD_KINDS`/`EntityKind` derived from `CARD_REGISTRY` (`isAnchoredCardKind`'s first consumers). Review **GO-WITH-NITS** (a live-import harness proved the derived set ≡ the old 13). A4 follow-up noted: `useAnchoredCard.ts` `as CardKind` cast (now done in A4-B).
  - **A10-D1** (`77c7c5c`) — accent SSOT: `ai`/`error` folded into `DEFAULT_PANEL_COLORS` + `SYSTEM_THEME_KEYS` (non-overridable). Review **GO**. (Body restyle → A9; collab `themeKey` → A10-rest.)
  - **A8** (`33d9b4a`) — five hand-synced printable-panel lists → one `PRINT_PANELS` SSOT; **`reports` now printable** (was the headline wart); `quotations` purged; redundant CSS allowlist + dead `data-print-p-*` deleted. Review **GO-WITH-NITS**. (Chrome-strip `data-card-chrome` marker → A9 / E-3; a live print-preview is a recommended fast-follow.)
- **A4 keystone (BATCH 1): Commits A–E landed on `chip-A4-selection-expansion`** (NOT merged; tsc + 570 tests green at every commit). The **store-side selection ⟂ expansion split is structurally complete** — `transient ∈ expanded` is gone; `anchored-card-store.ts` now has `{expandedSet, selected, hover}` + axis-pure primitives; the 13 cards' conflated click centralized in `useAnchoredCard.onActivate` (R1 select+expand); **marker clicks select WITHOUT expanding** (the selected-but-collapsed cell is now reachable); the three-surface halo paints from the single `selected` slot; the deprecated shims are deleted. **Remaining: Commit F (one-click expand chevron + docked popout button in the unified header) + Commit G (ErrorCard panel-local expand axis, R5), then dev-preview the 2×2 + independent review + merge.** Plan: `~/.claude/plans/scalable-mixing-whale.md`.
- **Two memory corrections this session:** (1) `release_prefs_snapshot_gotcha` — **do NOT run `tools/sync-defaults.sh` (even `--check`) for chip verification**; it auto-committed a stale-snapshot "Promote personal prefs" commit onto a branch (caught + reset; `main` safe). Eyeball the `*.defaults.json` diff by hand. (2) Review/impl workflows now carry a hard **no-working-tree-mutation guard** (forbidding `git checkout`/`stash`/`reset` + `sync-defaults`) — closes both the `git checkout` race and the auto-commit hazard.
- **Commit-message gotcha:** backticks in a double-quoted `git commit -m "…"` trigger shell command-substitution (silently drops the word). Use single quotes / no backticks.
- **Next: finish A4 (F, G) → review → merge; then BATCH 0's A1 + AF-follow; then BATCH 2 (A3 ∥ A2-rest ∥ A9) and BATCH 3 (A5 ∥ A6 ∥ A10-rest).** Run as a management session off workflows + dispatched chips (the Wave-2 audits + seam sweep are all landed — `WAVE2-seam-sweep.md` §5 has the sequencing, §6 has R1–R34).
...
- **BATCH-0 progress:** A2-B1 ✅ · **A10-D1 ✅ landed** (`77c7c5c`): ai/error accents folded into `DEFAULT_PANEL_COLORS` via `themeFromAccent` (same hexes → zero visual change) + `SYSTEM_THEME_KEYS` non-overridable across the `setPanelColor`/`loadPanelColors`/`clearPanelColor` trio + the picker filter; independent review **GO** (closed the "missing-JSON-key → black-theme" trap tsc can't catch; the promote-defaults merge is additive so the new keys can't be dropped at release). · **A8 ✅ landed** (`33d9b4a`): the five hand-synced printable-panel lists collapsed to one `PRINT_PANELS` SSOT — `reports` printable by construction (was unprintable), `quotations` purged + a dev canary, the redundant CSS allowlist + dead `data-print-p-*` stamping deleted (render-only gate, statically proven); review **GO-WITH-NITS** (no-mutation guard held; visual print-preview a recommended fast-follow). Remaining: **A1** (gardening + toolbar kill) · **AF-follow** (`snapshotForStack`) → then the A4 keystone (BATCH 1).
- **A8 follow-ups (small):** the `data-card-chrome` print-strip MARKER lands with A9 (E-3); a visual print-preview pass (reports prints, quotations gone, render-only gate) is the one recommended fast-follow; `chrome-config.ts` `ActionKind` legacy `comment`/`cut` tokens (A8 Finding 7) → A3/A10.
...
### Ratified (session 14 — BATCH-2 A9 gating subset)
- **R11 — Report typography = SANS/apparatus** *(Gabriel)*. Report is AI commentary, not borrowed document prose → `bodyClass:"sans"`, matching N2's "everything else → sans"; A9 fixed its declared-vs-rendered serif mismatch (a deliberate visible serif→sans correction).
- **R14 — note↔highlight morph = BIDIRECTIONAL chevron** *(Gabriel)*. Uniform with the other 3 pairs; the one-way `+note` button is replaced by the chevron; note→highlight is lossy with a confirm. (Follow-up: gate the chevron off for paragraph-only Mode-A notes — an empty-highlight UX smell, → A2/A6.)
- **R10 — borrowed-content cards KEEP in-card editing** *(Gabriel)*. `BorrowedMainText` only augments display/collapsed/reader/popped-read; footnote/note/archive stay editable (no behavior change).
- **R13 — morph shape = `{ to, lossy }` + a registered `convert`** *(self, technical)*; **R12 — highlight excerpt stays a faithful serif STRING** (no per-highlight TipTap) *(self)*; **R5 — the dead `ac.props` aria DELETED** *(self; A1-inherited)*.
...
| AF-follow | Real `Floatable.snapshotForStack()` + delete the legacy stack path (GAP-8) — **✅ landed** (`355c1ad`): mandatory per-kind snapshot, EditorPane repointed, `resolve-card.ts` deleted; review GO-WITH-NITS; tsc + **594 tests**. (`bib`/`ai` full `FloatChrome` deferred → A9.) | 3 | impl | ✅ **landed** (`355c1ad`) | `main` | AF |
| A1-impl | Gardening + R30 toolbar GC — **✅ landed** (`4fff0be`): R30 (proven dead-code, no live capability lost) · useComments · menuLocation · autoFitBody · ErrorCard lift-wiring · grip-drag · snap-grid · `kind:"toolbar"` union · shell trim · doc-comments. Review GO-WITH-NITS; tsc + **594 tests**. (`popCardAtAnchor`→A3; chrome-config→A8; `ac.props` aria→A9.) | 3 | impl | ✅ **landed** (`4fff0be`) | `main` | A0 · AF · AF-follow |
| A2-impl | Anchoring: B-1 `EntityKind` fold (`6697ad6`) + **A-rest ✅ landed** (`e39ffff`): `cardKindFromRecord` classifier · `isInlineAtomCardKind` · legacy-token crosswalk · derived `ANCHOR_CLICK_ROUTES`/`entityKindToAnchorKind` (R-B split). Review GO-WITH-NITS; tsc + **612 tests**. (`EntityCollections` fold → A3.) | 3 | impl | ✅ **landed** (`e39ffff`) | `main` | A0 · A1-reloc · A10-D1 |
| A8-impl | Print/reader: **printable-panel SSOT ✅ landed** (`33d9b4a`) — one `PRINT_PANELS` table; `reports` printable, `quotations` purged, the redundant CSS allowlist collapsed; chrome-strip marker rule awaits A9 (E-3) | 3 | impl | ✅ **landed** | `main` | A0 |
| A4-impl | **Keystone** — selection ⟂ expansion split (one-click expand + popout, marker-select). Commits **A–H ✅ MERGED** (`90c050e`): store split · body-click centralized · marker-select · halo→`selected` · shims deleted · F header controls (`isPoppable` SSOT) · G ErrorCard R5 · H review-nits (lift gated on `isPoppable`, axis-purity test). Review **GO-WITH-NITS**; tsc + **577 tests** green | 3 | impl | ✅ **landed** (`90c050e`) | `main` | A0 · AF · A2-B1 |
| A3-impl | Creation pipeline + lifecycle ratification; owns `popCardAtAnchor`; **+ the deferred `EntityCollections`→`CardCollections`/`CardFloatCtx` fold-then-consume** (A2-rest handoff) | 3 | impl | planned (BATCH 2 — **last arena**) | — | A4 · A2-rest (A–D ✓) |
| A9-impl | Borrowed-display + two-class typography + morph chevron — **✅ landed** (`f68fd79`): morph chevron all 4 pairs (docked+popped+omni, `convertCardWithRemap` float-survival) · N2 `bodyClass` typography · `BorrowedMainText` · R5/R10/R11/R12/R13/R14 ratified. Review GO-WITH-NITS (2 HIGH typography/display + 1 MED folded); tsc + **605 tests** | 3 | impl | ✅ **landed** (`f68fd79`) | `main` | A4 · A10-D1 |
