...
| AF-impl | `Floatable` subsystem *(`src/floats/`)* | 3 | impl | 🟠 **built — review NO-GO** | `chip-AF-floatable` (uncommitted) | A0 ✓ · Q1 unify-phased ✓ |
| AF-fix | Unify ALL DOM-key seams via one shared helper + contract tests (data-card-key ✓; omniKey / resolvePos / focus / CSS / morph-remap pending) | 3 | impl | 🟠 **in progress — NO-GO #2** | `chip-AF-floatable` | AF review NO-GO |
