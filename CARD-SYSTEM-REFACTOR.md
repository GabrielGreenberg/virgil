...
| A4-impl | **Keystone** — selection ⟂ expansion split (one-click expand + popout, marker-select). **Commits A–E ✅ on branch** (store split · body-click centralized · marker-select · halo→`selected` · shims deleted) · **F (header controls) + G (ErrorCard) pending**, then dev-preview + review + merge | 3 | impl | 🔄 **A–E on branch (NOT merged)** | `chip-A4-selection-expansion` | A0 · AF · A2-B1 |
