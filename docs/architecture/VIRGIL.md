...
Two honesty notes: `escapeLatex` escapes only `& % # _ ~ ^` and smart quotes — it deliberately leaves `\ { } $` as live syntax; and display math's *source* form is `\[…\]` (the `$$…$$` seen in the editor is a DOM/input-rule register, normalized on save). Directly-typed accent/dash/ellipsis glyphs (é, –, —, …) normalize-on-save back to canonical LaTeX commands via the same typography table (the smart-quote precedent). The library skills' [_latex-output.md](../../library/skills/_latex-output.md) constrains skill *output* to a curated subset — narrower than what the parser accepts.
