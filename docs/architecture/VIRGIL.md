...
*An orienting map. The authoritative how-to-work-on-it detail lives in the `docs/agents/*` derivatives — this section is the conceptual index those docs specialize. Verified against `docs/agents/overview.md` + `architecture.md` (both `last-verified: baf05ee5`).*
...
- **Nesting is one grammar-derived vocabulary** (task 338): `skipOpaqueConstructAt` ([src/lib/latex-lexer.ts](../../src/lib/latex-lexer.ts)) is the single answer to "which nested construct is opaque to my scan?" — any `\begin{env}`, the two expex pairs, an inline `\verb` run — comment-aware, and fail-soft on an unterminated construct. Every body splitter and all three environment terminators read it, so a nested env can no longer split its enclosing list or example.

Two honesty notes: `escapeLatex` escapes only `& % # _ ~ ^` and smart quotes — it deliberately leaves `\ { } $` as live syntax; and display math's *source* form is `\[…\]` (the `$$…$$` seen in the editor is a DOM/input-rule register, normalized on save). Directly-typed accent/dash/ellipsis glyphs (é, –, —, …) normalize-on-save back to canonical LaTeX commands via the same typography table (the smart-quote precedent). The cross-silo [_latex-allowlist.md](../../editor/skills/_latex-allowlist.md) (a byte-identical copy in each of `editor/skills/` and `library/skills/`, drift-guarded, that every `.tex`-writing skill links to) constrains skill *output* to a curated subset — narrower than what the parser accepts; `library/skills/_latex-output.md` is now the library deep-index appendix that transcludes it. A coherence check (`tools/check-coherence.mjs` #6 "allowlist") keeps the allowlist honest against the renderer's inline SSOT.
