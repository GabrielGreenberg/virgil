...
**Sibling subsystems at the repo root:** `library/` (the Library tab — its own components/hooks/lib/parser/store + Python skills; mounts the canonical `<EditorPane>`; reached via the `@library/*` alias; the recent UI/UX tune-up added `LibraryCentralDashboard.tsx` — the search-first stats dashboard the Central tab opens to, with a Dashboard|Browse switch — and the `PageScrollLozenge.tsx` "p. N" scroll lozenge that replaced the deleted page-scroll strip, backed by `lib/catalog-stats.ts`, `lib/catalog-search.ts` (a flexible catalog-search adapter onto the fuzzy bib search), `lib/list-window.ts` (a hand-rolled fixed-height list windower), and `lib/view-session-store.ts` (Library view-pref persistence); detail lives in [library/AGENTS.md](../../library/AGENTS.md)) and `editor/` (the editor-side skill bundle — `/editor/review` umbrella + per-kind subskills + Python helpers under `editor/scripts/`; bridged into the app via [src/lib/ai-request-bridge.ts](../../src/lib/ai-request-bridge.ts)).
