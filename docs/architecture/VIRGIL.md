...
- `src/hooks/` — ~50 state hooks (`useDocument`, `useCitations`, `useFootnotes`, `useViewPrefs`, `useCollab`, `usePoppedCards`, …), including `useReconcileModeAAnchors` (the Mode-A reload reconcile pass — UUID-first, paragraph-snapshot fallback, built on `resolveCardAnchor`).
- `src/lib/` — core logic: LaTeX parse/serialize, TipTap extensions, storage, types. Includes `actions/` (the `VIRGIL_ACTION_REGISTRY` action SSOT + the `editor-actions-bridge.ts` PM→React seam + `action-icons.tsx`), `focus-view.ts` (the UUID-anchored focus-band library + `focusViewPlugin`), the code-view libs (`code-position-map.ts` — the unified UUID↔LaTeX-source position map; `code-band.ts` — the code-side cursor band), `sanitize-html.ts` (the allowlist HTML sanitizer wired into the bibliography annotation editor, BIB-F5-01), `anchor-mint-signal.ts` (the `ANCHOR_MINT_META` tx-meta that triggers an immediate doc-bundle flush when an anchor UUID is minted, off the keystroke path), and the TipTap `anchor-highlight-deco.ts` (`AnchorHighlightDecorator` — paints card hover/selection on in-editor node/atom anchors via a `Decoration.node` plugin, eliminating the foreign-mutation redraw class; Mode-B text-range highlight stays on raw `setAttribute`).
- `src/links/` — the unified link architecture (registry, resolvers, three-surface highlight reconcilers). Includes the **anchor-recovery SSOT**: `resolve-card-anchor.ts` (`resolveCardAnchor` — the pure UUID→mark→snapshot→orphan resolver + `buildResolveIndex` + `reconcileCardToResolved`), and `_shared/reapply-mode-b-anchors.ts` (the single load-time Mode-B re-apply pass that retired `EditorLayout.applyLinkedAnchors` → one `EditorPane` recovery pass) + `_shared/normalize-text.ts` (`normalizeParagraphText`, normalize-at-capture for the self-healing paragraph snapshots).
