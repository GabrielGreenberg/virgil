...
| 2 | **Type accounting** | **error** | **Both directions.** Every exported type in `src/lib/types.ts` is accounted for in VIRGIL.md's [Public-type registry](VIRGIL.md#public-type-registry) section, or explicitly delegated to a named manifest doc — AND every type name the registry, its delegated sections, or a doc deriving from it backticks is exported by a registry `covers-code` source (or declared `type-externals`), AND every stated `**N** exported` count is the real one. (Graduated to error: Phase 0 filled the registry; the reverse + count arms landed in task 562.) |
...
**Staging (as built):** Phase 0 (chip 4) filled the registry section and removed its stub marker, so the check runs as designed: **per-type error** on any genuinely-unaccounted type. The graduation trigger has fired. One implementation note: VIRGIL.md's registry section names ~25 types directly but **delegates the full 58-type enumeration** to the manifest's `sidecars.md` (its [Coverage](../workspace/sidecars.md#coverage) index) via a prose link. The script follows any `.md#anchor` link inside the registry section and harvests backtick type-names from the linked section too — so the delegation resolves mechanically and every exported type is accounted. (Through chip 9 this delegated to a Phase 0 seed report; chip 10 retired the seeds and repointed it to the manifest.)

**The reverse direction (task 562).** As designed above the check was ONE-directional — it asked whether every export is NAMED and never whether every NAME still EXISTS — so a type deleted from `types.ts` stayed enumerated in the skill-facing manifest forever (three of them did: two removed in A1 gardening, one in task 417, with full field-level schemas still shipping in `sidecars.md`), and a `**58** exported` count sat two exports stale in three places, all with the check green. The same tautology task 260 found in `assertActionCoverage`, one direction over. So check 2 now also asserts, over the same **naming surfaces** (the registry section, every local `.md#section` it links, and — new — the whole body of every graph node whose `derives-from` targets the registry section, since a doc derived from the type registry is a type-naming doc and the stale `comments.json` schema sat OUTSIDE the delegated Coverage section):

- **REVERSE** — every PascalCase backtick token on a surface is exported by one of the registry *section's own* `covers-code` sources (the registry covers `src/panels/_shared/types.ts` and `src/links/_shared/types.ts` beside the SSOT, which is how `Link` resolves with no special case), or is declared by the naming doc's `<!-- type-externals: Name (why) -->` header. Anything else is a deleted/renamed type still in prose → **error**, naming the doc, the section and the token.
- **STALE DECLARATION** — a `type-externals` name that a covers-code source DOES export, or that the doc names nowhere, is an exemption that has stopped excusing anything → **error** (an allowlist inside a guard may only shrink).
- **COUNT** — every `**N** exported` a surface states must equal the SSOT's real export count → **error**. The number is the one thing on these surfaces a reader takes as a fact about code, and it is the thing that rotted.

The one PascalCase token that is neither a type nor a stale name — TipTap's `JSONContent`, which the manifest names as the body format — is declared once in `sidecars.md`'s header. Smoke fixtures E–J in `check-coherence.smoke.mjs` drive both directions, the count pin, the external/stale-external arms and the section-in-whole-doc dedupe; check 2 had no smoke coverage at all before this.
...
- **Check 2 (types):** **error** (graduated), **both directions** since task 562. Phase 0 filled the registry and removed the `<!-- STUB -->` marker, so per-type erroring on genuinely-unaccounted types is live; the reverse arm (a named type that no longer exists), the stale-external arm and the stated-count pin are errors too. The script no longer keys off the marker (there are none); check 2 is wired as a per-type error directly, and currently passes clean (every export accounted via the §2 delegation, every name resolving).
