...
>
> **Residual:** `--par-title-color: #c45a5a` still sits outside the scale, deliberately — it is the paragraph-title *concept* colour rather than an alarm, so folding it in is a visual decision, not a sweep. The Outline panel's two borrowed `#b45757` spellings were drained by task **2026-08-02-284** (verified 2026-08-14): the InlineLabel conflict ink took `--danger-muted`, the twin its in-prose `.heading-label-input.has-conflict` already read, and the current-position wash stopped borrowing the rust altogether — "where the caret is" is `--accent`'s role, not a footnote's — so both allowlist entries retired with it.
