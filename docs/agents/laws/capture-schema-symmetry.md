...
The SSOT is [`fitNodeInContainer`](../../../src/text-objects/container-fit.ts) (pure, schema-level) behind [`fitNodesAtInsert`](../../../src/components/drop-mode/specs/drop-context.ts) (editor-level), a four-rung ladder: **direct** where the immediate parent accepts the bare node → **wrap** where a wrapper in the registry-derived wrap vocabulary (`WRAP_TARGET_KINDS`, read from `parentKinds`; task 776) is both valid at that index *and* able to hold the node → **direct** where a probe shows the fitter only pads → **reject**. Four rules it earned:
