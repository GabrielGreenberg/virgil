...
**The importer half (task 768).** A `read-fresh` claim was checked only against the lib's own file, so `collab.ts` passed while `useCollab` snapshotted `loadIdentity()` into `useState` and never re-read it — a peer window's "Edit identity" never arrived, and that window kept heartbeating and claiming the pen under the old name (`useRowDotState` did the same to the viewed map). The claim is now also checked against IMPORTERS: an importer that puts one of the lib's LOADERS (an exported function whose own body reads `localStorage`; pure helpers exported beside it don't count) into React state — `useState(() => load())`, `useRef(load())`, `setX(… load() …)` — without subscribing makes `read-fresh` false. Reading fresh at the moment of use (a dialog prefilling from `loadIdentity()`) is not a cache. Neutered against the pre-768 hooks it names exactly those two.
