...
**[src/components/Editor.tsx](../../src/components/Editor.tsx)** (~1880 lines) wraps TipTap's `useEditor`. Since the FCU refactor it builds its extension list from the shared `buildEditorExtensions(ctx)` factory in [src/lib/editor-extensions.ts](../../src/lib/editor-extensions.ts) (the same factory every float editor consumes), then layers on keyboard shortcuts, selection handling, custom plugins, and wires up `onUpdate` → parent. Also mounts the gutter `SelectionActionsMenu` (lightning-bolt button → `ActionsMenuPanel`) at the editor root.
...
- **Unanchored dock**: a card whose anchor resolves to `source:'orphan'` (uuid + mark + snapshot all dead) can't be line-aligned, so instead of culling it the gutter surfaces it in a fixed "unanchored — click to re-pin" dock (the `unanchored` flag on `MarginaliaMarker`).
