...
- **Footnote-owned citations nest.** A `\cite` that lives *inside* a footnote's body is tagged `nestedInFootnoteId` on its `CitationEntry` by the `DocStructureObserver`, so it renders **nested under its footnote's card** in the docked Citations panel / omni rather than as a free-standing citation card (footnote-nesting Phase 2). References have no card, so there's nothing to nest there.
