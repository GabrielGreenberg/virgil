...
**Verbatim content is BYTE-LITERAL, and one mark says so.** Two carriers exist for raw LaTeX the editor doesn't model, and the difference is load-bearing:

- **`latexCommand`** — raw LaTeX, but prose-adjacent. Its serializer path deliberately runs `smartenStraightQuotes`, because TipTap inherits this mark onto stray text the user types beside a command, and that text still has to emit valid `.tex`.
- **`latexVerbatim`** — *byte-literal*. An inline `\verb<delim>…<delim>` run, or a `VERBATIM_ENVS_FULL` environment with no modeled node (`verbatim*` / `lstlisting` / `minted` — bare `verbatim` has the richer `codeBlock`). Every serializer returns this text **exactly as parsed**: no char-escaping, no typographic reverse-map. The mark spec is also `code: true`, so TipTap's input/paste-rule runner declines inside it — the type-time half of the same law.

The carrier, the `matchInlineVerbAt` inline-verb matcher, and `VERBATIM_ENVS_FULL` itself live together in [src/lib/latex-lexer.ts](../../src/lib/latex-lexer.ts); the mark is registered in [src/lib/tiptap/latex-command.ts](../../src/lib/tiptap/latex-command.ts), where it is also `inclusive: false` — text typed at the trailing edge must NOT inherit a carrier that has neither a type-time nor a save-time smart-quote net. **Both** inline parsers read the shared matcher — the main one and the footnote/card fork in `footnote-content.ts` — so they can't drift on where a `\verb` run starts or ends. The *environment* half is main-parser only: the fork has no block parsing at all, so a verbatim ENV pasted into a footnote body still takes the prose path.

Known gaps, deliberately out of task 264's scope: the inline siblings `\lstinline` / `\mintinline` / `\path`, the `\url`/`\href` argument, and the verbatim environments outside the four-member SSOT (fancyvrb `Verbatim`, `alltt`, beamer `semiverbatim`, `filecontents`) all still ride the smart-quoting `latexCommand` path with the identical defect. The carrier makes each of them a small, local fix — a vocabulary addition, not an architecture change.
