...
The carrier, the `matchInlineVerbAt` inline-verb matcher, and `VERBATIM_ENVS_FULL` itself live together in [src/lib/latex-lexer.ts](../../src/lib/latex-lexer.ts); the mark is registered in [src/lib/tiptap/latex-command.ts](../../src/lib/tiptap/latex-command.ts). **Both** inline parsers read them — the main one and the footnote/card fork in `footnote-content.ts` — so the two can't drift on what counts as verbatim.
