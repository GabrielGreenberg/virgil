...
- **Creating a citation = a deferred-commit POPOVER** (the shared create controller — see below). The explicit create surfaces (slash `\cite`, the lightning grid cell, the grab-bar 'Citation' action) open [`CitationCreatePopover`](../../src/panels/Citations/CitationCreatePopover.tsx) OVER THE TEXT at the caret (cursor auto-focused in a citekey search). Picking citekeys STAGES them (chips); the real `\cite{…}` atom + gutter card materialize only on commit (the OK button, or click-away / Escape) and only when ≥1 key is staged — so the gutter never flashes a blank pristine card mid-pick. The popover reuses [`CitekeyPicker`](../../src/panels/Citations/CitekeyPicker.tsx) (`keepOpenOnPick`) for the paper-bib + library search. **Typed LaTeX is the exception**: `\cite{key}` (full) and `\cite ` / `\citep ` (bare) stay atom-first — the user typed the exact command, so it inserts synchronously, preserving the command verbatim. Subsequent edits (prenote/postnote, command type, add/remove keys) happen in the gutter card at its standard position.
