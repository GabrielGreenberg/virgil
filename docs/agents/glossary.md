...
| **Card theme** | `CARD_THEMES` dict (footnote, note, archive, todo, bib, citation, comment, aiRequest, cut, error, report) | Same file |
...
| User term | Code name(s) | Where |
|---|---|---|
| **Card-creation helpers** (unified create-from-selection flow) | `cardCreation` context with `.createNote`, `.createCut`, `.createTodo`, `.createFootnote`, `.createCitation`, `.createReport`, `.createReportRequest` | [src/components/editor-layout/contexts/card-creation.tsx](../../src/components/editor-layout/contexts/card-creation.tsx) + [src/components/editor-layout/card-actions/card-creation.ts](../../src/components/editor-layout/card-actions/card-creation.ts) |
