...
| **Figure block** (`\begin{figure}…\end{figure}` environment rendered as an embedded image pod with structured attrs: raw / source[] / widthPercent / caption / label) | TipTap node `figureBlock` (atom block); click opens `FigurePopover` for in-place tex-mode editing | [src/lib/tiptap/figure-block.ts](../../src/lib/tiptap/figure-block.ts); node-view [src/components/FigureBlockNodeView.tsx](../../src/components/FigureBlockNodeView.tsx); popover [src/components/FigurePopover.tsx](../../src/components/FigurePopover.tsx); attr parsing [src/lib/figures/parse-attrs.ts](../../src/lib/figures/parse-attrs.ts) |
...
| **Figure popover** (tex-mode editor that opens on click of a figure / graphics block; lets the user edit the raw LaTeX, with the renderer re-rasterizing on save) | `FigurePopover` | [src/components/FigurePopover.tsx](../../src/components/FigurePopover.tsx); save handler in `EditorLayout.tsx` (~line 2670) routes through `extractFigureAttrs` / `extractGraphicsAttrs` |
