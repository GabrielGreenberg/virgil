...
| `revisions` | Revisions | `comment` (key prefix `revision`) + `revision-suggestion` (polymorphic — `card.kind` is `comment` in registry; `revision-suggestion` registered in `CARD_KEY_PREFIXES` and added to `POLYMORPHIC_CARD_PANEL` in 3fa5996). Comment cards now use `EditableCard` chrome (rich-text body, AI-request checkbox in footer); each card carries a header chevron that converts in-place between comment ↔ revision-suggestion via `convertCard`. `CARD_TYPE_LABELS` renamed both `suggestion` and `revision-suggestion` to "Revision" (3fa5996) | right (omni-eligible) | stone |
