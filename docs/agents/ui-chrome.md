...
- **[src/components/EditorPane.tsx](../../src/components/EditorPane.tsx)** (~7840 lines) — the **canonical editor surface** mounted by both the main app's doc branch (from EditorLayout) and the Library Reader (from `library/components/PaperRender.tsx`). EditorPane owns per-doc hooks (`useDocument`, `useLatexCompile`, `useNotes`, `useTodos`, `useCitations`, `useCollab`, `usePristineCardManager`, …), the docked `MenuBar` (now inside the in-card chrome header, ~line 6019 — see MenuBar section), the panel rail (`PaneRail` left + right), the floating-panel block, and the canonical `DockOutline` (~line 5236) / `CardLiftOutline`.
...
│   └─ DockOutline (body-portaled drag-target outline, suppressed in zen) — EditorPane.tsx:5236
