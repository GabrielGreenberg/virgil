...
selecting the fresh card doesn't trip `usePlacement`'s card→text
`alignEntryToYIfNeeded` scroll (one call covers all card kinds —
footnote/citation were the reported jumps). Since task 328 that door is the
gated one: a card already fully visible in its band holds rather than scrolls
(`src/lib/reposition-policy.ts`), and raw `alignEntryToY` is callable only from
`layout-scroll.ts` (CI: `gutter-stability-census.test.ts`). (2) Inline-atom creates (footnote/citation/`\ref`/inline-math) route
...
- **The `DragHandleMenu` lifecycle actions** — `duplicate` (clone the captured
  ...
  `archive`, and `delete` (Family 1). The cascade is Mode-B-anchor-wide, not a
  hand-picked list: since task 721 `todo` / `report` / `report-request` carry
  `{ clone: true, delete: true, bindAnchor: true }` like the other Mode-B kinds, so
  duplicating a passage carries its todo and its report (it used to drop both
  silently while carrying its note), and a cascade delete of a flagged card closes
  its `ai-requests.json` row through the unbridging wrappers.
...
remain their own producers — as does one PANEL-INTERNAL native drag: an Outline pod
reorder, which carries the private `MIME_OUTLINE_POD` and **no `text/plain`**, and
which the editor refuses BY NAME via `EDITOR_REFUSED_DRAG_TYPES`
([marginalia.ts](../../src/lib/marginalia.ts), task 708), so releasing a pod over the
paper types nothing into it. A re-anchor through a drop adapter must answer what it
...
changing the document. There are exactly **seven** `DecorationSet` plugin sources (an
...
see below). One of the seven, the transient highlight, mounts one plugin INSTANCE
per **channel** (task 667 — `search` and `ai-request` today):
...
| **Transient highlight** | a partial-block text band painted as a `Decoration.inline` — a view-only signal that is NEVER a mark/attr (task 120 "transient state is never document content"). Two CHANNELS since task 667, each its own plugin instance, key and set: `search` (search hit, diagnostics error range, quoted revision) and `ai-request` (the light-blue wash over an OPEN AI request's anchored region, formerly a real `linkedAnchor` mark — see [request-wash.ts](../../src/links/_shared/request-wash.ts)). A target may declare `inclusive: true` for a band denoting a live REGION rather than a fixed range | inline | [transient-highlight.ts](../../src/lib/tiptap/transient-highlight.ts) (`setTransientHighlights(view, targets, channel)`/`clearTransientHighlights`) | Compliant — a channel rebuilds ONLY on its OWN meta tx (send that channel's complete desired set per frame; `[]` clears it and touches no other channel); every other tx just `map(tr.mapping, tr.doc)`s the set, and the meta tx is `!tr.docChanged` so no history/autosave/bus |
...
itself — hence the mark-presence rebuild trigger in the row above. Stated
residual: a literal backslash that arrived from a source `\textbackslash{}` is
still painted grey by the bare-text decoration although it emits escaped.
