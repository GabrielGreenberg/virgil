...
|---|---|---|---|
| The anchored **block** is deleted | anchor (A) | **`TextObjectOrphanGuard`** emits `virgil-textobject-orphaned`; **`MarginaliaAnchorGuard`** *pre-empts* it for marginalia-bearing blocks by re-inserting a **placeholder paragraph with the same uuid** at the deletion site. Two stand-downs: a transaction tagged `LIFECYCLE_DELETE_META` (the Archive / Delete drag-handle actions), where the removal is deliberate; and — since task 367 — a removal its own remedy would reproduce byte-for-byte (`removed.eq(paraType.create({ uuid }))`, i.e. the block was ALREADY an empty uuid-only paragraph). Restoring there preserves nothing and silently VETOES the gesture, which is why Backspace on such a husk did nothing, forever, for every press; the guard fails OPEN (an unreadable position, or a uuid that no longer matches, still resurrects). Since task 491 the two `LIFECYCLE_DELETE_META` gestures no longer behave alike: a DELETE removes the context, so a card pointing at it has nowhere to be and orphans as before, but an ARCHIVE sets the text ASIDE (it still exists, one panel over), so every **Mode-A** card the capture displaces RE-HOMES onto the surviving neighbour — the same neighbour the fresh snippet anchors to, resolved ONCE per gesture by `resolveDisplacedAnchorTarget` ([src/text-objects/anchor-resolution.ts](../../src/text-objects/anchor-resolution.ts)) and applied by [src/cards/retarget-anchors.ts](../../src/cards/retarget-anchors.ts). **Mode-B** anchors keep the old path: their anchor names the text RANGE, and the range is precisely what left | `recoverOrphanedUuids` re-attaches a sidecar id by **unique** content fingerprint |
