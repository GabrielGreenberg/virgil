...
|---|---|---|---|
| `%!v:<4hex>` | a **TextObject** (paragraph, heading, list, listItem, blockquote, codeBlock, displayMath, latexComment, titleField, figureBlock, graphicsBlock, …) | trailing line-end **comment** (not a macro) | **appended to the END of the construct's serialized body** and detached from the same place — `appendUuidAnchor` / `detachUuidAnchor` in `src/lib/uuid.ts` are exact inverses, and nothing spells the token by hand (`uuidAnchorToken` / `uuidAnchorSuffix`, task 348). A construct's last line may therefore stack its last child's anchor before its own (`\end{itemize} %!v:child %!v:me`); the detach takes exactly ONE, greedy-prefixed, so the LAST wins. `detachItemAnchor` adds one narrow legacy branch for `listItem`s written before 348. Task 387 added the corollary for a construct whose body is a **user-editable attr**: `forestBlock`'s `source` is stored verbatim by both pod write doors, so the serializer trims its trailing whitespace before appending — otherwise a stray newline after `\end{forest}` de-anchors the block silently and every card, marginalia marker and sidecar `parTitle` keyed on that uuid follows the stranded anchor onto a blank line. |
| `%!v:blank` | an empty, unidentified paragraph | sentinel comment | round-trips to/from an empty paragraph. |
| `\vfid{<4hex>}` | a **footnote** Atom | inline no-op macro | emitted immediately **before** `\footnote{}` / `\thanks{}`; parser parks it in a `PendingMarkerId` (`src/lib/latex-markers.ts`) and it binds to the atom at **exactly** the position the marker left off — an unclaimed marker is dropped, never handed to a later footnote (task 341). |
| `\vcid{<4hex>}` | a **citation** Atom | inline no-op macro | emitted **before** the cite command; same positional `PendingMarkerId` binding as `\vfid`. |
...
- **Everything else that declares a `uuid` attr** gets one **by default** — headings,
  ...
  maketitleMarker). Read from the SSOT, not from a hand list, so a new
  uuid-bearing kind mints instead of being silently skipped (task 343).
...
On parse the marker is parked with its position and claimed only by an Atom
starting exactly there: the Atom's `footnoteId` ← `\vfid`, `citationId` ←
`\vcid`. **This id is the Atom-link key**: the footnote Card's `id` in `footnotes.json` must equal the
...
- `UUID_BEARING_NODE_TYPES` lives in `src/lib/node-attr-sets.ts` (with
  `TITLED_NODE_TYPES`, `COLLAPSIBLE_NODE_TYPES` and `DEFERRING_PARENTS` /
  `deferringParent`) — an import-free leaf the TipTap-free `.tex` layers can
  read, **CI-checked against the real editor schema**, so a schema addition
  fails the build instead of going write-only (tasks 343 / 346). `needsUuidWork`
  (the save-path gate that decides whether `assignUuids` runs at all) reads the
  same set.
