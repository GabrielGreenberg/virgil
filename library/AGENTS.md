...
> **This file and `library/CLAUDE.md` are two files, and must stay two files.** Claude Code nested-loads a `CLAUDE.md` into every session that reads anything under its folder, so the sibling is a ~560-byte POINTER here — not a copy, and never a symlink. It was a symlink once: on 2026-09-15 a commit meaning to shrink `library/CLAUDE.md` wrote the pointer text *through* it and destroyed all 93 KB of this guide, leaving the symlink blob byte-identical in the diff so the change looked like a no-op. The ceiling-only budget guard stayed green for four days, because a file that is gone satisfies every ceiling. Both halves are now pinned by [src/__tests__/agents-md-budget.test.ts](../src/__tests__/agents-md-budget.test.ts): a two-sided `{ min, max }` envelope over every tracked guide (this file has a 60 KB FLOOR; the pointer a 2 KB ceiling), a leg asserting no guide is a symlink or shares an inode with another, and a census that every citation naming a section of this file — anywhere in `src/`, `library/`, `docs/agents/` or `editor/` — resolves to a section that still exists — so the guardrail escape hatches below cannot dangle again.
...
The lock primitives live in [library/scripts/_tools.py](scripts/_tools.py)
