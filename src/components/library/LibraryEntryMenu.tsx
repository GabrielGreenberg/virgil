 ...
 *
 * Reusable across callsites:
 *   - Bibliography panel: pick an entry to add to the paper's local bib.
 *   - (Future) +citation in the editor: pick an entry to insert as \cite.
 *
 * Each callsite supplies its own `onPick` handler and (optionally) a
 * `getRowState` that classifies each entry as addable / added / conflict
 * for that callsite's notion of "already there".
 */
...
import { searchCentralLibrary } from "@/lib/bib-search";
...
export interface LibraryEntryMenuProps {
  ...
  /** Caret-coord / floating-position anchor (future +citation callsite). */
...
export function LibraryEntryMenu(props: LibraryEntryMenuProps) {
  ...
  return <LibraryEntryMenuInner {...props} />;
}
...
function LibraryEntryMenuInner({
  anchorEl,
  anchorRect,
  onClose,
  onPick,
  ...
  /** Local override of getRowState — when a pick completes we promote a
   *  row to "added" without forcing the parent to re-derive its state
   *  every render. */
  ...
  const filtered = useMemo(() => {
    if (!query.trim()) return libraryBibEntries.slice(0, 50);
    return searchCentralLibrary(libraryBibEntries, query);
  }, [libraryBibEntries, query]);

  // Clamp selectedIndex on every render so a query that shrinks the list
  // can't strand it past the end. Resetting to 0 on every new keystroke
  // happens directly in the input's onChange to avoid effect-driven setState.
  ...
  const performPick = useCallback(
    async (entry: BibEntry) => {
      const initial = rowStateFor(entry);
      if (initial === "added") return;
  ...
  const onKeyDown = useCallback(
    ...
    [filtered, safeSelectedIndex, performPick, onClose],
  );
  ...
  return createPortal(
    <div
      ref={popupRef}
      role="dialog"
      aria-label="Search the central library"
      className="library-entry-menu bg-surface border border-edge-subtle rounded-md shadow-md"
      style={{
        position: "fixed",
        left,
        top,
    ...
    >
      ...
      >
        {filtered.length === 0 ? (
          <div className="px-3 py-4 text-[11px] text-ink-muted text-center">
            {query.trim()
              ? `No library entries match "${query}".`
              : libraryBibEntries.length === 0
                ? "Library is empty."
          ...
          filtered.map((entry, i) => (
            <LibraryEntryRow
              ...
              libraryItem={libraryByCitekey.get(entry.key)}
              membershipChips={membershipChipsFor({
                inLocal: false,
...
interface RowProps {
  ...
  membershipChips: ReturnType<typeof membershipChipsFor>;
  onHover: () => void;
  onToggleExpand: () => void;
  onPickClick: () => void;
}
...
function LibraryEntryRow({
  ...
  return (
    <div
      ...
      className={`group relative px-2.5 py-1.5 cursor-default ${
...
function AddButton({
  ...
  if (state === "added") {
    return (
      <span
        ...
        title="Already in your bibliography"
      >
        <svg
          width="11"
          height="11"
  ...
  const title = state === "conflict" ? "Add — citekey conflict" : "Add to bibliography";
  return (
    <button
      type="button"
      onClick={(e) => {
