...
//
// ONE bus, two PUBLISHERS. The consumer set is identical, and the second
...
// differ (pointer edges vs a resize burst with no `resizestart` event), and
// they are colocated here because `beginLayoutGesture`/`endLayoutGesture` are
...
export type LayoutGestureKind = "pane" | "window";
 ...
 *  drag in either silo, or an OS window resize. Read from an observer
