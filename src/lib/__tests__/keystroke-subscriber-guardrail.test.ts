...
const PERMITTED_KEYSTROKE_SUBSCRIBERS: Record<string, string> = {
  "components/EditorLayout.tsx":
    "[cost: O(1)/tx; deferred body O(headings) fast-path, O(doc) flag-off fallback] Three subscribers: activity-presence counter bump (docChanged-gated, mounted only while iHavePen); section-path recompute main + mirror ('update' → RAF-coalesced + perf-flag gate; the deferred compute's PRIMARY path is the Wave-2 C2 geometry derivation computeSectionPathAt — ONE posAtCoords + binary search over the DocStructure snapshot behind geomBreadcrumbEnabled() — with the legacy coordsAtPos doc-walk surviving only as the virgil:geom-breadcrumb flag-off/service-null fallback; the resize path is gesture-parked via LAYOUT_SITE_SECTION_PATH*, scroll stays live). (The PDF-stale bump was removed in P6 — pdfStale is owned solely by EditorPane.)",
  "components/EditorMirror.tsx":
    "[cost: O(1)/tx; deferred body O(edit) replay] RAF-deferred mirror replay — the transaction handler only schedules a frame.",
