...
const PERMITTED_RESIZE_LISTENERS: Record<string, string> = {
  ...
  "src/components/EditorLayout.tsx":
    "PARK ×2 — the Help-menu anchor rect (left-anchored Virgil-bar chrome, displaced ~0.001·delta by a width drag, so a gesture-stale anchor cannot visibly detach the popover) and the main section-path breadcrumb walk (O(headings) `coordsAtPos`, ProseMirror's most expensive forced-layout call; the MIRROR pane's twin park retired with the editor split in task 115). Since task 416 BOTH geometry paths park through the same site — the scroll path's 'a breadcrumb must follow the scroll it describes' argument was true of a USER scroll (which is not a layout gesture, so the park is a no-op for it) and blind to the scroll a CONTENT drag performs. See `scroll-listener-guardrail.test.ts`.",
  ...
  "src/components/editor-layout/reader-view-prefs.ts":
    "PARK — the second copy of the breadcrumb walk (main pane, Reader; the mirror pane's copy retired with the editor split in task 115), same O(headings) `coordsAtPos` cost and — since task 416 — the same BOTH-paths-park shape as EditorLayout's one.",
  ...
  "src/hooks/useInTextPositions.ts":
    "PARK — the in-text marker deck re-measures every tracked position; both geometry triggers (window + the editor-dom RO) park, while the structural-bus and font-ready paths stay live because they aren't gesture-driven. Since task 370 the parked scheduler's end-edge settle enters the shared convergence door rather than firing a lone pass, so a gesture still costs ONE settle — but that settle now runs to the consumer's fixed point (two passes agreeing within the task-328 hysteresis) instead of stopping on one frame of unchanged scrollHeight. The park is what keeps this off the per-frame path; the convergence is what keeps the one settle honest.",
  "src/lib/editor-geometry/service.ts":
    "PARK the MEASURE PASS, never the accumulation (the marginalia engine, moved verbatim from useMarginaliaRegistry into the editor-attached geometry service — wave 2 C4) — the highest FIRE-COUNT follower during a horizontal drag (a width change rewraps every paragraph, so the per-block RO delivers one entry per near-zone block per frame). `pendingRecompute` keeps collecting across the gesture (that set IS the work list); only the RAF measure pass defers, so the gesture costs ONE flush over the union. Mid-gesture the per-entry `invalidateFromUuid` — O(docOrder) per entry, i.e. O(blocks²) per frame at 300+ blocks — collapses to one O(observed) all-dirty mark. The SAME listener also fires the VIEWPORT-FRAME park (C7 — the refresh that was useEditorViewportCache's, absorbed here: a getComputedStyle + 4× getBoundingClientRect pass whose equality bail cannot hold mid-gesture, formerly mounted ×4 live with 4 private listeners; now ONE parked settle per gesture for the whole pane). Task 416 gave the IO half the same treatment — mid-gesture an ENTER defers its measure onto the same `pendingRecompute` work list and both exits route through a park, so a content drag's auto-scroll costs one settle instead of a repack per crossed near-zone boundary.",
  ...
  "src/text-objects/TextObjectGrabHandle.tsx":
    "PARK — the grab handle's placement re-solve rides the shared park; the handle is left-anchored chrome (`handle-layout.ts`), which a width drag displaces sub-pixel. Task 416 routed its SCROLL path through the same park (a content drag DOES deliver pointer events, so the hover branch was answerable and re-solving per auto-scroll frame).",
};
