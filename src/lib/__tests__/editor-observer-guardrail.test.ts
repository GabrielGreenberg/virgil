...
const PERMITTED_RESIZE_OBSERVERS: Record<string, string> = {
  ...
  "components/chrome/tab-strip-occupancy.ts":
    "[cost: per fire = one `contentRect.width` compare (width-equality bail) then a scheduled frame; per coalesced FRAME = ONE active-tab rect pair + at most one `scrollLeft` write; ZERO fires during a layout gesture (parked via parkDuringLayoutGesture, settles once on the end edge)] ONE observer per tab strip (both the Virgil bar's and the Library's inner strips mount `useTabStripScroller`), over the strip's own SCROLLER box only — it exists so a strip that narrows under the active tab scrolls the tab back into view (task 561). It observes no editor content and writes no style: its only write is the scroller's `scrollLeft`, which does not resize the box it observes, so it cannot feed back into itself.",
  "components/editor-layout/useBarOccupancy.ts":
    "[cost: per fire, three `entry.contentRect.width` reads (post-layout — forces NO layout) behind a per-role equality bail, then one pure arithmetic predicate; per commit, ONE boolean of React state, written only when it flips. The ROW's entry additionally reads O(tabs) `scrollWidth`/`clientWidth` pairs off its label spans (`tabRowNaturalWidth`, task 561) — post-layout properties on the same fire, forcing no layout — to recover the row's NATURAL width from a compressed box] ONE observer for the whole Virgil bar, over three boxes: the tab strip's SCROLLER (its assigned box), the tab ROW, and the tools group's `max-content` wrapper. It writes no style and no CSS var, so there is nothing for its own writes to feed back into; the one thing it CAN change (collapsing the tools group) moves `tabStripPx` by exactly the term the predicate cancels, so it converges in a single step by construction rather than by hysteresis (bar-occupancy.ts). Deliberately NOT parked on the layout-gesture bus, for `useWindowChrome`'s reason: the frame itself is the obligation — park the verdict and the bar visibly overlaps for the whole of an OS window drag, which is the defect it exists to close. It does NOT observe editor content, and the boxes it does observe move only on tab open/close/rename, a window resize, and a self-gating status badge changing width — the last of which the save pill does on its tier edge, so a typing burst fires this at most once, for the same O(1). (The first draft of this entry claimed it observes nothing that changes while typing, which was the stronger claim and false.)",
  ...
  "lib/forest/measure-watch.ts":
    "ONE observer app-wide (the card-near-zone shape), over each mounted forest tree's own host. Its callback is a `contentRect.width <= 0` compare plus a `degraded()` read, and it bumps a counter only for a host that ADMITS its last measure fell back to estimates — a tree measured from real boxes ignores every fire, including the initial one `observe` delivers. It reads no layout (`contentRect` is delivered post-layout) and it exists because un-hiding is not an event the component can see: a `forestBlock` inside a folded section stays MOUNTED with no box (the fold is a node DECORATION), so its first layout is placed from canvas estimates and no effect dependency ever changes when the section is unfolded. The 0 → non-zero box IS the signal.",
  "panels/Outline/OutlinePanel.tsx":
