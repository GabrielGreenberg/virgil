...
describe("canMorphNoteToHighlight (A6/WS7)", () => {
  ...
  it("the registry morph declaration stays static (note⇄highlight, lossy)", () => {
    // The gate is consumer-owned; the declared pair must NOT change.
    expect(CARD_REGISTRY.note.morph).toEqual({ to: "highlight", lossy: true });
    expect(CARD_REGISTRY.highlight.morph).toEqual({ to: "note", lossy: true });
  });
});
