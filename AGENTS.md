...
Riding it: `panel-theme.ts`, `panel-typography.ts`, `outline-prefs-store.ts`, `useStack`, `useStyleLibrary`. Cross-window-safe by other means: `useViewPrefs` (the `global-pref-changed` bus in `src/lib/multi-window/bus.ts`), and the no-module-cache helpers that re-read per call (`style-library`, `library-store`, `row-viewed-store`, `list-columns`). **Still stale-snapshot stores, not yet migrated** (each writes a whole blob from a cached base): `usePreferences`, `useZenMode`, `pref-links`, `useWordCountConfig`, `library/lib/view-session-store`, `useLibraryTabs`, the `ActionsMenuPanel` palette.
