...
- **It is a net keystroke REDUCTION, and that is the honest reason it is cheap.** `findParagraphPos` was a full `descendants` walk PER PID, so the omni pass cost O(doc · anchors) per items rebuild; the authority builds ONE `buildResolveIndex` and resolves each card in O(1). Both hosts memoize it on the DocStructureBus counters (`rev.anchors` / `rev.blocks`) plus the reactive editor, so plain typing rebuilds nothing. On the margin side the index also moved OUT of the marker memo, so editing a note's title no longer re-walks the document.
- **`uuidToPos` rides the walk `buildResolveIndex` already does.** The index's own `descendants` pass visits every uuid-bearing node WITH its position, so the position map costs one `Map.set` per node and no second walk — which is what made retiring `findParagraphPos` free rather than a trade.
...
The leg with teeth is the CENSUS ([card-anchor-authority-census.test.ts](src/links/__tests__/card-anchor-authority-census.test.ts)) — the authority was never the part that could misbehave, a call site that never asks it is, and `findParagraphPos(pid)` type-checks perfectly while answering the wrong question. Membership is DISCOVERED from the panels tree, so a new panel is covered by existing: no `omni.tsx` may declare a private position lookup (one exemption, per file with its reason — the Errors builder's paragraph id comes from the diagnostics pass, not from a card's links, so it has no ladder to run and no second renderer to agree with) or read a card's raw stored pids; both hosts must build the pass and the margin must read it through the shared reader; and nothing outside the authority and the load-time `useReconcileModeAAnchors` MUTATOR may call the recovery ladder at all. Measured on the pre-369 tree, the four legs name six builders twice, `EditorPane`, and both hosts.
