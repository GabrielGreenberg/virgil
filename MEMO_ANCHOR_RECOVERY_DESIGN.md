...
**Wave 2 sequencing (SEQUENTIAL to avoid `EditorPane.tsx`/`links.ts` collisions):**
- **W2a — RC-A+D (combined, full-adversarial):** resolver-driven load reconcile (swap body, keep data-loss-safe shell) + augmentations 1–3 above. Files: `useReconcileModeAAnchors.ts`, `links.ts` (reconcile + captureParagraphSnapshot normalize), `resolve-card-anchor.ts` (optional live-text arg if needed), `EditorPane.tsx` (reconcile call site only), tests.
- **W2b — B (full-adversarial) ∥ C (single-skeptic)** after W2a: B = render resolver-driven marker + snapshot fallback + orphan surfacing (use `resolver.source==='orphan'`, avoid touching `links.ts isModeAOrphaned`) + registry observe-miss fix; C = commit-flush. Disjoint `EditorPane.tsx` regions (B ~1742 memo; C ~1358 drop bag).
- **W3 — RC-B (full-adversarial, alone):** retire `EditorLayout.applyLinkedAnchors`, move Mode-B re-apply into the EditorPane pass (resolver `liveAnchorId`-driven, highlights LAST). Highest blast.
