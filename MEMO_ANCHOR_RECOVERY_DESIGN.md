...
**STATUS: ✅ CORE COMPLETE — all 6 chips merged to local `main` (`d09853c`). Full suite 1789/1789, tsc 0. NOT pushed.** Merge sequence: R0 `b7ec2c6` → A `51f2b86` → RC-A+D `fe7a64c` → RC-B `62458f9` → C `52b2396` → B `d09853c`. RC4 (the 3-owner divergence) is collapsed into the single EditorPane load pass; the racing second WRITER (`EditorLayout.applyLinkedAnchors`) is deleted. Every chip adversarially reviewed (skeptic or multi-lens workflow); CHIP-B's review caught + fixed a multi-anchor-marker regression before merge. Worktrees cleaned. **Remaining: deferred follow-ups only (E5 / CK / RPT / polish batch — none block the bug).**
...
**Nits to clean in a later POLISH chip (batched, non-blocking — all behavior is correct):** (a) misleading comment in `text-object-side-reanchor.ts` (says highlight "no-ops" — only `clearModeB` no-ops; `addTextObjectLink` still adds a link); (b) CHIP-A's e2e harness mutates a synchronous `noteApi`, not the live `useNotes` hook (verified harmless out-of-band); (c) [CHIP-C] coalescing comment `useDocument.ts:312-316` + `controller.ts` overstates "applyDrop doesn't touch the doc" — false for the Mode-B mark-strip (benign: the strip correctly triggers its own flush); narrow the wording; (d) [CHIP-C] add a test rejecting `mockWrite` on the prior save (proves stale `lastSavedRef` → commit-flush still fires, the 1(b) durability guarantee) + a Mode-B mark-strip commit-flush test.
...
- **W2b — RC-B ✅ MERGED `62458f9`** (multi-lens review SHIP-WITH-NITS, no blockers; full suite 1771/1771, tsc 0). Doc-side recovery unified into the single EditorPane pass; `EditorLayout.applyLinkedAnchors` effect deleted. **RC4 now resolved on BOTH sides (sidecar RC-A + doc RC-B).**
- **W2c — C ✅ MERGED `52b2396`** (single-skeptic SHIP-WITH-NITS; crux dedupe-suppression cleared). RC3 closed. **B ✅ MERGED `d09853c`** (multi-lens review BLOCK→fix→delta-review SHIP; cures the ~10s vanish, RC2; bounded observe-miss RAF retry; OrphanDock new UI). Final whole-suite gate 1789/1789, tsc 0.
- **Deferred:** `E5` (dual-mount reader retirement), `CK` (legacy-token normalization), **`RPT` (reports/report-request Mode-B mark not re-applied on load — PRE-EXISTING gap surfaced by RC-B's review, not a regression; possibly nonexistent if reports can't be selection-anchored; verify reports support Mode-B before chipping).**
