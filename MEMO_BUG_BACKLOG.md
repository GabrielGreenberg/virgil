...
### (1) Citations inside EXAMPLES nest under the example card (like footnotes) — `NEEDS-DESIGN-DECISION` — see `MEMO_FOOTNOTE_CHILD_CARD_NESTING.md` §5a
...
### (2) Restore the one-off whitespace lead-in above the title — `PLAN-READY` (⚠️ self-inflicted regression) — see `MEMO_TOPBOTTOM_MARGIN_GAP.md` §10 + correction banner
...
### (3) At max top-margin adjuster, text should reach the pod edge — `IMPLEMENTED`/live-verify — see `MEMO_TOPBOTTOM_MARGIN_GAP.md` §10
